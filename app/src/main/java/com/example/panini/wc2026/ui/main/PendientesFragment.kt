...
class PendientesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val adapter = LaminaAdapter(TipoLista.PENDIENTES) { lamina ->
            // "Obtener" → registrarla como obtenida
            viewModel.registrarLaminaObtenida(lamina)
        }

        binding.recyclerView.layoutManager = LinearLayoutManager(requireContext())
        binding.recyclerView.adapter = adapter
        binding.tvTitulo.text = "Láminas Pendientes"

        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.laminasPendientes.collectLatest { lista ->
                adapter.submitList(lista)
                binding.tvVacia.visibility = if (lista.isEmpty()) View.VISIBLE else View.GONE
                binding.tvVacia.text = if (lista.isEmpty()) "🎉 ¡Álbum completo!" else ""
            }
        }
    }
